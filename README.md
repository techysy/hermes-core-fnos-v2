...
### 1. 预构建内核（构建机 31.31，需 python3.12 + Node）
```bash
bash scripts/prebuild.sh    # 产出 app/venv.tar.gz（v0.20.0 venv + web_dist）
```
...
### 3. 安装
在飞牛 NAS 应用中心手动安装 `HermesCore-*.fpk`。

## 目录结构
```
├── manifest              # fnOS 清单（v0.9.9）
├── VERSION               # 0.9.9
...
├── scripts/
│   ├── prebuild.sh       # 预构建 v0.20.0 venv + web_dist
